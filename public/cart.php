...
function cart(){
    ...
    $product=<<<DELIMETER
<tr>
 <td>{$row['product_title']}<br><img width="62px;"src="{$row['product_image']}" alt=""></td>
         <td>£{$row['product_price']}</td>
         <td>{$value}</td>
         <td>{$sub}</td>
         <td class="text-center"><a class='btn btn-warning' href="./cart.php?remove={$row['product_id']}"><span class='glyphicon glyphicon-minus'></span></a>   <a class='btn btn-success' href="./cart.php?add={$row['product_id']}"><span class='glyphicon glyphicon-plus'></span></a>
...
function report(){
    ...
    echo "Item Quantity: ".$item_quantity."<br><br>";
    echo "Total Price: ".$total+=$sub;      
   }
  }
  }session_destroy();
}else{
